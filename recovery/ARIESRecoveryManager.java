...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        transactionTable.get(transNum).transaction.setStatus(COMMITTING);
        return newLSN;
    }
    ...
    private void rollbackToLSN(long transNum, long LSN) {
        ...
        while (currentLSN > LSN) {
            ...
            if (currRecord.isUndoable()) {
                LogRecord cLRRecord = currRecord.undo(transactionEntry.lastLSN);
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        if (!this.dirtyPageTable.containsKey(pageNum)) {
            this.dirtyPageTable.put(pageNum, newLSN);
        }
        return newLSN;
    }
    ...
    public synchronized void checkpoint() {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry: transactionTable.entrySet()) {
            if (!fitsInOneRecord(numDPTRecords, numTxnTableRecords + 1)) {
                LogRecord record = new EndCheckpointLogRecord(chkptDPT, chkptTxnTable);
                logManager.appendToLog(record);
                flushToLSN(record.getLSN());
                chkptDPT.clear();
            ...
            chkptTxnTable.put(entry.getKey(), new Pair<>(status, lastLSN));
            numTxnTableRecords++;
        }
    ...
    void restartAnalysis() {
        ...
        while (log.hasNext()) {
            ...
            if (record.getTransNum().isPresent()) {
                Long transNum = record.getTransNum().orElse(-1L);
                ...
                if (record.getType() == COMMIT_TRANSACTION) {
                    tableEntry.transaction.setStatus(COMMITTING);
                } else if (record.getType() == ABORT_TRANSACTION) {
                    tableEntry.transaction.setStatus(RECOVERY_ABORTING);
                } else if (record.getType() == END_TRANSACTION) {
                    tableEntry.transaction.cleanup();
                    tableEntry.transaction.setStatus(COMPLETE);
            ...
            if (record.getPageNum().isPresent()) {
                ...
                if (Arrays.asList(LogType.UPDATE_PAGE, LogType.UNDO_UPDATE_PAGE).contains(record.getType())) {
                    if (!dirtyPageTable.containsKey(pageNum)) {
                        dirtyPageTable.put(pageNum, record.getLSN());
                    }
                } else if (Arrays.asList(LogType.FREE_PAGE, LogType.UNDO_ALLOC_PAGE).contains(record.getType())) {
                    dirtyPageTable.remove(pageNum);
                }
            }

            if (record.getType() == END_CHECKPOINT) {
                ...
                for (Map.Entry<Long, Pair<Transaction.Status, Long>> entry : txnTable.entrySet()) {
                    ...
                    if (endedTransactions.contains(transNum)) {
                        continue;
                    } else if (!transactionTable.containsKey(transNum)) {
                        Transaction transaction = newTransaction.apply(transNum);
                        startTransaction(transaction);
                    }
                    ...
                    Transaction.Status inMemoryStatus = transactionTable.get(transNum).transaction.getStatus();
                    Transaction.Status chkptStatus = entry.getValue().getFirst();
                    if (memoryStatusOutOfDate(inMemoryStatus, chkptStatus)) {
                        if (chkptStatus == ABORTING) {
                            chkptStatus = RECOVERY_ABORTING;
                        }
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            ...
            if (status == COMMITTING) {
                ...
                entry.getValue().transaction.setStatus(COMPLETE);
                ...
                entry.getValue().transaction.setStatus(RECOVERY_ABORTING);
                LogRecord endRecord = new AbortTransactionLogRecord(entry.getKey(), entry.getValue().lastLSN);
                entry.getValue().lastLSN = logManager.appendToLog(endRecord);
            }
        }

        for (Long transNum : toRemove) {
            transactionTable.remove(transNum);
        }
    }

    private boolean memoryStatusOutOfDate(Transaction.Status inMemoryStatus, Transaction.Status chkptStatus) {
        if (inMemoryStatus == Transaction.Status.RUNNING) {
            return true;
        } else if (inMemoryStatus == Transaction.Status.ABORTING
                && chkptStatus != Transaction.Status.RUNNING) {
            return true;
        } else if (inMemoryStatus == COMMITTING
                && chkptStatus == Transaction.Status.COMPLETE) {
            return true;
        }
        return false;
    }
     ...
     */
    void restartRedo() {
        if (dirtyPageTable.values().isEmpty()) return;
        ...
        while (log.hasNext()) {
            ...
            if (Arrays.asList(ALLOC_PART, FREE_PART, UNDO_ALLOC_PART, UNDO_FREE_PART,
                    ALLOC_PAGE, UNDO_FREE_PAGE).contains(record.getType())) {
                record.redo(this, diskSpaceManager, bufferManager);
            } else if (Arrays.asList(UPDATE_PAGE, UNDO_UPDATE_PAGE, UNDO_ALLOC_PAGE,
                    FREE_PAGE).contains(record.getType())) {
                Long pageNum = record.getPageNum().orElse(-1L);
                Long pageLSN;
    ...
    void restartUndo() {
        ...
        for (TransactionTableEntry entry : transactionTable.values()) {
            if (entry.transaction.getStatus() == RECOVERY_ABORTING) {
        ...
        while(!pq.isEmpty()) {
            ...
            if (record.isUndoable()) {
                ...
                LogRecord cLRRecord = record.undo(entry.lastLSN);
            ...
            if (record.getUndoNextLSN().isPresent()) {
                nextLSN = record.getUndoNextLSN().orElse(-1L);
            } else {
                nextLSN = record.getPrevLSN().orElse(-1L);
            }
