...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        List<Integer> indices = this.getEligibleIndexColumns(table);
        int minIndex = -1;
        for (int i = 0; i < indices.size(); i++) {
            SelectPredicate predicate = this.selectPredicates.get(indices.get(i));
            QueryOperator indexScan = new IndexScanOperator(
                    this.transaction, table, predicate.column, predicate.operator, predicate.value);
            if (indexScan.estimateIOCost() < minOp.estimateIOCost()) {
                minIndex = i;
                minOp = indexScan;
            }
        }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Map.Entry<Set<String>, QueryOperator> pair : prevMap.entrySet()) {
            for (int j = 0; j < this.joinPredicates.size(); j++) {
               JoinPredicate predicate = joinPredicates.get(j);
               Set currSet = pair.getKey();
               Set<String> newSet = new HashSet<>();
               newSet.addAll(currSet);
               if (currSet.contains(predicate.leftTable) && !currSet.contains(predicate.rightTable)) {
                   QueryOperator op = pass1Map.get(new HashSet<>(Arrays.asList(predicate.rightTable)));
                   newSet.add(predicate.rightTable);
                   result.put(newSet, minCostJoinType(pair.getValue(), op, predicate.leftColumn, predicate.rightColumn));
               } else if (!currSet.contains(predicate.leftTable) && currSet.contains(predicate.rightTable)) {
                   QueryOperator op = pass1Map.get(new HashSet<>(Arrays.asList(predicate.leftTable)));
                   newSet.add(predicate.leftTable);
                   result.put(newSet, minCostJoinType(op, pair.getValue(), predicate.leftColumn, predicate.rightColumn));
               }
            }
        }
        return result;
    ...
    public Iterator<Record> execute() {
        ...
        for (int i = 0; i < tableNames.size(); i++) {
            QueryOperator op = minCostSingleAccess(tableNames.get(i));
            pass1Map.put(new HashSet<>(Arrays.asList(tableNames.get(i))), op);
        }
